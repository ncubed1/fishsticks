...
export default class ObjectsInterface {
  ...
  px2m(px) {
    return {
      x: px.x / this.s / this.canvas.pixiCanvas.scale.x,
      y: -px.y / this.s / this.canvas.pixiCanvas.scale.y,
    };
  }
  ...
  zoom(deltaDiff, pos, deltaPos, prevDiff) {
    ...
    let { x: factorX, y: factorY } = {
      x: (prevDiff + deltaDiff) / prevDiff,
      y: (prevDiff + deltaDiff) / prevDiff,
    };
    ...
    this.objects.moveCanvasBy({
      x: (pos.x - this.canvas.pixiCanvas.x) * (1 - factorX) + deltaPos.x,
      y: (pos.y - this.canvas.pixiCanvas.y) * (1 - factorY) + deltaPos.y,
    });
  }
